...
class PostController extends BaseController
{
	...
	public function getLimitedPosts(Request $request)
    {
        ...
        // Retrieve the posts with the specified limit
    ...
    {
        // Step 1: Get all categories matching the criteria
        $categories = Category::where('id', $category_id)
            ->orWhere('parent_id', $category_id)
            ->get();

        // Extract category IDs for further use
        $categoryIds = $categories->pluck('id')->toArray();

        // Step 2: Retrieve posts for the selected categories
        $isDetailed = (request()->filled('detailed') && (int)request()->get('detailed') == 1);

        $query = Post::whereIn('category_id', $categoryIds);

        if ($isDetailed) {
            $defaultEmbed = ['user', 'category', 'postType', 'city', 'savedByLoggedUser', 'pictures', 'latestPayment'];

            // Check if the "package" relationship exists on the Post model
            if (method_exists(Post::class, 'package')) {
                $defaultEmbed[] = 'package';
            }

            // Check if the "extra" relationship exists on the Post model
            if (method_exists(Post::class, 'extra')) {
                $defaultEmbed[] = 'extra';
            }
            ...
            $posts = $query->with($defaultEmbed)->get()->map(function ($post) {
    ...
    public function getByCity($city_id)
    {
        ...
        $query = Post::query()->where('city_id', $city_id);
    ...
    public function getByUser($user_id)
    {
        ...
        $query = Post::query()->where('user_id', $user_id);
