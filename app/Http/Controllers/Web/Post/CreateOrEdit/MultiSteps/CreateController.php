...
class CreateController extends FrontController
{
    ...
    public function getPostStep(Request $request)
    {
        ...
        $postData = http_build_query(['data' => json_encode($postInput)]);
        $checkURL = 'https://gtsjo.net/HarajAPI/CheckText/check?' . $postData;

        // Send the data and check the response
        $response = Http::get($checkURL);
        $responseData = $response->body();

        if ($response->status() === 200 && $responseData === '1') {
            // Stop and return an error message
            return "Validation failed. The data contains prohibited words.";
        }

        // Continue with the rest of the code if the validation passes
    ...
    public function postPostStep(PostRequest $request)
    {
        ...
        // Prepare the data to send
        $postData = http_build_query(['data' => json_encode($postInput)]);
       // $checkURL = 'http://SDS-Drivers.somee.com/API/ValidateString?' . $postData;
        $checkURL = 'http://gtsjo.net/HarajAPI/CheckText/check?text=sample+text' . $postData;
        ...
        if ($response->status() === 200 && $responseData === '1') {
            // Stop and return an error message
            return "Validation failed. Those data contain prohibited words.";
        } else {
